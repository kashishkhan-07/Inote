...
const NoteCard = ({ note, onEdit, onDelete }) => {
  ...
  return (
    <div className="group relative bg-white dark:bg-slate-800/90 rounded-2xl p-6 border border-slate-200/80 dark:border-slate-700/60 hover:border-indigo-200 dark:hover:border-indigo-500/40 shadow-xs hover:shadow-xl dark:shadow-slate-900/50 hover:-translate-y-1 transition-all duration-200 flex flex-col justify-between">

      <div>
        {/* Card Header: Title & Copy */}
        <div className="flex items-start justify-between gap-3 mb-3">
          <h3 className="font-semibold text-slate-900 dark:text-white text-lg leading-snug line-clamp-2">
            {note.title}
          </h3>
          ...
          <button
            onClick={handleCopy}
            title="Copy Note"
            className="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/60 p-1.5 rounded-lg transition-colors cursor-pointer shrink-0"
          >
            {copied ? (
      ...
      {/* Card Footer */}
      <div className="pt-4 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between text-xs text-slate-400 dark:text-slate-400">
        ...
        <div className="flex items-center gap-1.5" title={isUpdated ? `Updated ${formatDate(note.updatedAt)}` : `Created ${formatDate(note.createdAt)}`}>
          ...
          {isUpdated && (
            <span className="text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-300 font-medium px-1.5 py-0.5 rounded-full">
              edited
            </span>
          )}
        </div>
